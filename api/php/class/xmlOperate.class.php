...
class xmlOperate{
	...
	function __construct($baseData){
		...
		$this->baseType = $baseData["type"];
		$this->resultArray = array();
		$this->argsArray = array();
		$this->typeArray = array();
		$this->baseDir = "../../../flyreadEditor/base/";
		$this->baseXmlPath = $this->baseDir."baseXml/";
		$this->baseJsonPath = $this->baseDir."data/";
		$this->baseResultXmlPath = $this->baseDir."xml/";
		$this->jPath = $this->baseDir."other/json/";
		$this->jsonPath = $this->baseJsonPath.$this->baseType."/".$this->baseName.".js";
		//$this->xmlPath = $this->baseXmlPath.$this->baseType."/".$this->baseName.".xml";
		$this->xmlPath = $baseData["path"];
		$this->resultXmlPath = $this->baseResultXmlPath.$this->baseType."/".$this->baseName.".xml";
	}
	function main(){
	...
	function updateXml($newXmlData){
        $tmpDom = $this->getXml($this->xmlPath);
        $root = $tmpDom->documentElement;
        if(isset($newXmlData["type"])&&$newXmlData["type"] == "node"){
            if(isset($newXmlData["attr"])){
                foreach($newXmlData["attr"] as $key => $val){
                    $root->setAttribute($key,$val);
                }
            }
            $args = $root->getElementsByTagName("args")->item(0);
            if(isset($newXmlData["args"])){
                foreach($newXmlData["args"] as $key => $val){
                    $args->setAttribute($key,$val);
                }
            }
        }
        $tmpDom->save($this->xmlPath);
        $this->resultArray = $this->getNodeData($root);
        return $this->resultArray;
	}
	function createJson(){
		$argsArray = $this->argsArray;
		$argsJsonString = $this->arrayToJs($argsArray);
		$fileAddress = $this->jsonPath;
		$this->writeFile($fileAddress,$argsJsonString);
		return $fileAddress;
	}
	function arrayToJs($argsArray){
		$string = "";
		$beginString = "{";
		$endString = "}";
		foreach($argsArray as $key=>$value){
			if(is_array($value)){
				switch($key){
					default:
						$string = $string.$key." : ".$this->arrayToJs($value).",";
					break;
				}
			}else{
		...
		$resultString = $beginString.$string.$endString;
		return $resultString;
	}
	...
	function createTestXml(){
		$nodeData = $this->resultArray;
		$nodeChilds = $nodeData["childs"];
		$addScript = "if(fatherId){tempArgs={};tempArgs[\"parent\"]=Ext.getCmp(fatherId);Ext.getCmp(fatherId).addObject(".$nodeData["attr"]["id"].",tempArgs);}";
		$tmpChild = array();
		$tmpChild["args"] = array();
		$tmpChild["childs"] = array();
		$tmpChild["type"] = "script";
		$tmpChild["value"] = $addScript;
		array_unshift($nodeChilds,$tmpChild);
		$nodeData["childs"] = $nodeChilds;
		$this->resultArray = $nodeData;
		$resultTestXmlPath = $this->createXml();
		return $resultTestXmlPath;
	}
	function createXml(){
		$nodeData = $this->resultArray;
		...
		$define = $this->createDefine($nodeData);
		$root->appendChild($define);
		$importArray = $this->createImports();
		if($importArray){
			for($i=0;$i<count($importArray);$i++){
				$root->appendChild($importArray[$i]);	
			}
		}
		$node = $this->createNode($nodeData);
		$root->appendChild($node);
		$this->xml->save($this->resultXmlPath);
...
//		$mergeArray = array_merge($firstArray,$secondArray);
	...
	function getC($value){
		...
		$cPath = $this->baseDir."other/columns/".$tmpValue.".js";
	...
	function getF($value){
		$tmpValue = substr($value,3);
		$fPath = $this->baseDir."other/function/".$tmpValue.".js";
	...
	function getP($value){
		$tmpValue = substr($value,3);
		$pPath = $this->baseDir."other/proxy/".$tmpValue.".js";
	...
	function getS($value){
		$tmpValue = substr($value,3);
		$sPath = $this->baseDir."other/store/".$tmpValue.".js";
	...
	function getT($value){
		$tmpValue = substr($value,3);
		$tPath = $this->baseDir."other/toolbar/".$tmpValue.".js";
